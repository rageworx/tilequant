...
(
	...
	struct RGBAf_t *TileValue = TilesData->TileValue;
	struct RGBAf_t *PxData    = TilesData->PxData;
	for(ty=0;ty<nTileY;ty++) 
        for(tx=0;tx<nTileX;tx++) 
        {
            //! Copy pixels as YUV, and get mean
            struct RGBAf_t Mean = {0,0,0,0};
            for(py=0;py<TileH;py++) for(px=0;px<TileW;px++) 
            {
                //! Convert and store pixel
                struct RGBAf_t Px = RGBAf_AsYUV(&PxRGBA[(ty*TileH+py)*(nTileX*TileW) + (tx*TileW+px)]);
                *PxData++ = Px;
                Mean = RGBAf_Add(&Mean, &Px);
            }
            ...
            (TilePxPtr++)->PxRGBAf = PxData - TileW*TileH;
            *TileValue++ = Mean;
        }
}
...
(
    ...
    (
		...
		DATA_ALIGN(nTiles*sizeof(struct RGBAf_t)) + //! TileValue
		DATA_ALIGN(nPx   *sizeof(struct RGBAf_t)) + //! PxData
		DATA_ALIGN(nPx   *sizeof(struct RGBAf_t)) + //! PxTemp
	...
	TilesData->TileValue  = (struct RGBAf_t*)DATA_ALIGN(TilesData->TilePxPtr + nTiles);
	TilesData->PxData     = (struct RGBAf_t*)DATA_ALIGN(TilesData->TileValue + nTiles);
	TilesData->PxTemp     = (struct RGBAf_t*)DATA_ALIGN(TilesData->PxData    + nPx);
...
int TilesData_QuantizePalettes(
    ...
    {
		struct RGBAf_t *PxTemp = TilesData->PxTemp;

		//! Get all pixels of all tiles falling into this palette
		int PxCnt; 
        {
			struct RGBAf_t *Dst = PxTemp;
            
			for(j=0;j<nTiles;j++) 
                if(TilesData->TilePalIdx[j] == i) 
                {
                    const struct RGBAf_t *Src = TilesData->TilePxPtr[j].PxRGBAf;
		...
		for(j=0;j<PalUnusedEntries;j++) *Palette++ = (struct RGBAf_t){0,0,0,0};
